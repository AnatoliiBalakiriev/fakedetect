...
def check_fakeness(article_text):
    ...
    if re.search(r'\bФейк|фейк|Манипуляция|манипуляция\b', article_text, re.IGNORECASE):
        return 1
    else:
        return 0
...
def fetch_article(url):
    ...
    for paragraph in article_content.find_all('p'):
        if "Источник" in paragraph.get_text():
...
def fetch_data_from_category(category, conn):
    ...
    for page_number in tqdm(range(1, 2)):  # Смужка прогресу для сторінок
        url = f'https://www.stopfake.org/ru/category/{category}/page/{page_number}/'
        response = requests.get(url)
        soup = BeautifulSoup(response.content, 'html.parser')
...
if st.sidebar.button("RUN"):
    ...
    if connection:
        ...
        for article_id, title, article, cosine_similarity, url in top_2_relevant_articles:
