...
def check_fakeness(article_text):
    ...
    if re.search(r'\bFake|Manipulation|Photo fake|Video fake|Photofake|Videofake\b', article_text, re.IGNORECASE):
        return 1
    else:
        return 0
...
def insert_data(conn, data):
    ...
    insert_query = """
    INSERT INTO pgml.stopfakes_en (title, article, url, date, relative_urls, source_url, anchor_texts, relative_images, fakeness)
...
def fetch_data_from_category(category, conn):
    ...
    for page_number in tqdm(range(1, 2)):  # Смужка прогресу для сторінок
        url = f'https://www.stopfake.org/en/category/{category}/page/{page_number}/'
        response = requests.get(url)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        for article in soup.find_all('div', {'class': 'item-details'}):
            ...
            if link_tag:
                ...
                with conn.cursor() as cursor:
                    cursor.execute("SELECT COUNT(*) FROM pgml.stopfakes_en WHERE title = %s", (title,))
...
def main():
    ...
    try:
        ...
        articles_data_kontekst = fetch_data_from_category("context", connection)
        if articles_data_kontekst:
            insert_data(connection, articles_data_kontekst)

        # Збір даних з категорії "factcheck_for_facebook_ru"
        articles_data_factcheck = fetch_data_from_category("factcheck-facebook-en", connection)
...
def create_embeddings_for_articles(conn):
    with conn.cursor() as cursor:
        ...
        cursor.execute("SELECT id, title || article AS title_article FROM pgml.stopfakes_en WHERE embed IS NULL;")
        ...
        for article_id, article in tqdm(articles, desc="Creating Embeddings"):
            ...
            cursor.execute("""
                UPDATE pgml.stopfakes_en
                SET embed = pgml.embed('WhereIsAI/UAE-Large-V1', %s)::vector(1024)
                WHERE id = %s;
            """, (article, article_id,))
...
def get_top_2_relevant_articles(conn, query):
    with conn.cursor() as cursor:
        ...
        cursor.execute("""
            WITH request AS (
                SELECT pgml.embed('WhereIsAI/UAE-Large-V1', %s)::vector(1024) AS query_vector
            )
            SELECT
                id,
                article,
                1 - (embed <=> (SELECT query_vector FROM request)) AS cosine_similarity
            FROM pgml.stopfakes_en
