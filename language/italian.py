...
def check_fakeness(article_text):
    ...
    if re.search(r'\bFalso|Manipolazione|video falso|videofake|Bugia|bugia\b', article_text, re.IGNORECASE):
        return 1
    else:
        return 0
...
def insert_data(conn, data):
    ...
    insert_query = """
    INSERT INTO pgml.stopfakes_it (title, article, url, date, relative_urls, source_url, anchor_texts, relative_images, fakeness)
...
def fetch_data_from_category(category, conn):
    ...
    for page_number in tqdm(range(1, 2)):  # Смужка прогресу для сторінок
        url = f'https://www.stopfake.org/it/category/{category}/page/{page_number}/'
        response = requests.get(url)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        for article in soup.find_all('div', {'class': 'item-details'}):
            ...
            if link_tag:
                ...
                with conn.cursor() as cursor:
                    cursor.execute("SELECT COUNT(*) FROM pgml.stopfakes_it WHERE title = %s", (title,))
...
def main():
    ...
    try:
        ...
        articles_data_kontekst = fetch_data_from_category("cronaca", connection)
        if articles_data_kontekst:
            insert_data(connection, articles_data_kontekst)

        # Збір даних з категорії "factcheck_for_facebook_ru"
        articles_data_factcheck = fetch_data_from_category("notizie", connection)
...
def create_embeddings_for_articles(conn):
    with conn.cursor() as cursor:
        ...
        cursor.execute("SELECT id, title || article AS title_article FROM pgml.stopfakes_it WHERE embed IS NULL;")
